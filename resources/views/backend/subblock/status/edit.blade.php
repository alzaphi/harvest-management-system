...
<div class="vendor-container">
    ...
    <form action="{{ route('status-sub-blocks.update', $statusSubBlock->id) }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')
                ...
                <div class="row">
                            ...
                            <select name="kode_petak" id="kode_petak" class="form-select" required>
                                ...
                                @foreach($subBlocks as $subBlock)
                                    <option value="{{ $subBlock->kode_petak }}"
                                            ...
                                            {{ $statusSubBlock->kode_petak == $subBlock->kode_petak ? 'selected' : '' }}>
                                        {{ $subBlock->kode_petak }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <input type="text" id="luas_area" class="form-control" readonly value="{{ $statusSubBlock->subBlock->luas_area ?? '' }}">
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <input type="text" id="estate" class="form-control" readonly value="{{ $statusSubBlock->subBlock->estate ?? '' }}">
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            <label class="form-label">Divisi</label>
                            <input type="text" id="divisi" class="form-control" readonly value="{{ $statusSubBlock->subBlock->divisi ?? '' }}">
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <select name="status" id="status" class="form-select" required>
                                ...
                                <option value="Planned Cutting" {{ $statusSubBlock->status == 'Planned Cutting' ? 'selected' : '' }}>Planned Cutting</option>
                                <option value="Already Cut Down" {{ $statusSubBlock->status == 'Already Cut Down' ? 'selected' : '' }}>Already Cut Down</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <div class="input-group">
                                <input type="number" step="0.01" name="luas_status" id="luas_status" class="form-control" required value="{{ old('luas_status', $statusSubBlock->luas_status) }}">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <input type="date" name="tanggal_update" id="tanggal_update" class="form-control" required value="{{ old('tanggal_update', $statusSubBlock->tanggal_update->format('Y-m-d')) }}">
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            <div class="form-check mt-4 pt-2">
                                <input type="checkbox" name="aktif" id="aktif" class="form-check-input" {{ $statusSubBlock->aktif ? 'checked' : '' }}>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    kodePetakSelect.addEventListener('change', function() {
        ...
        if (selectedOption.value) {
            ...
            // Auto-fill luas_status with luas_area if it's empty
    ...
    // Trigger change event to update fields if kode_petak is already selected
