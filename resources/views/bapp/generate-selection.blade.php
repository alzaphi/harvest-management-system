...
@section('page-title', 'Pilih Data Hasil Tebang untuk BAPP - ' . $vendor->nama_vendor)
...
<div class="container mx-auto px-4 py-8">
    <div class="bg-white rounded-lg shadow-md p-6">
        <h2 class="text-2xl font-semibold mb-6">Pilih Data Hasil Tebang untuk BAPP Tebang - {{ $vendor->nama_vendor }} ({{ $vendor->kode_vendor }})</h2>
        ...
        <form action="{{ route('bapp.confirm') }}" method="POST" id="bappForm">
            @csrf
            <input type="hidden" name="vendor_kode" value="{{ $vendor->kode_vendor }}">
            
            @if($hasilTebang->count() > 0)
                        ...
                        <tbody>
                            @foreach($hasilTebang as $item)
                                <tr>
                                    ...
                                    <td>
                                        <span class="status-badge {{ $item->status === 'Selesai' ? 'status-generated' : 'status-not-generated' }}">
                                            {{ $item->status ?? 'Proses' }}
                                        </span>
                                    </td>
                                </tr>
                ...
                <div class="mt-6 flex justify-between items-center">
                    ...
                    <button type="submit" class="btn-generate" id="generateBtn" disabled>
                        Lanjutkan Konfirmasi BAPP Tebang
                    </button>
                </div>
            @else
                <div class="alert alert-warning">
                    <p>Tidak ada hasil tebangan yang tersedia untuk dibuatkan BAPP.</p>
                </div>
                <a href="{{ route('hasil-tebang.show', $vendor->kode_vendor) }}" class="btn-back">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Update generate button state based on checked checkboxes
        function updateGenerateButton() {
            const checkedBoxes = document.querySelectorAll('.hasil-checkbox:checked');
